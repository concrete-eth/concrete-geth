...
func (eth *Ethereum) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	...
	for current.NumberU64() < origin {
		...
		root, err := statedb.CommitWithConcrete(
			eth.blockchain.Concrete().PrecompiledAddressesSet(current.NumberU64()),
			current.NumberU64(),
...
func (eth *Ethereum) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (*core.Message, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		statedb.FinaliseWithConcrete(vmenv.ConcretePrecompiledAddressesSet(), vmenv.ChainConfig().IsEIP158(block.Number()))
	}
